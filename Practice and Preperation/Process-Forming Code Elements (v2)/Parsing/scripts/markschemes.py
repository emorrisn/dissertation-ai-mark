...
AGGREGATE_PATTERN = re.compile(r"^m(?P<session>\d+)\.markscheme\.txt$", re.IGNORECASE)
...
class MarkschemeProcessor:
    ...
    def _finalize_sessions(self) -> List[str]:
        ...
        sessions_with_artifacts = set(remaining.keys())
        ...
        for session, paths in remaining.items():
            ...
            if len(outstanding) == 0:
                self._finalize_single_session(session, ready_sessions)

        for aggregate in self.data_dir.glob("m*.markscheme.txt"):
            session = self._parse_aggregate_session(aggregate)
            if not session or session in sessions_with_artifacts or session in ready_sessions:
                continue
            self._finalize_single_session(session, ready_sessions)

        return ready_sessions

    def _parse_aggregate_session(self, aggregate_path: Path) -> Optional[str]:
        match = AGGREGATE_PATTERN.match(aggregate_path.name)
        if not match:
            return None
        return match.group("session")

    def _finalize_single_session(self, session: str, ready_sessions: List[str]) -> None:
        aggregate_in_data = self.data_dir / f"m{session}.markscheme.txt"
        if not aggregate_in_data.exists():
            return
        try:
